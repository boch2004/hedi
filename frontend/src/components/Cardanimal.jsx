...
function Cardanimal({ animal, ping, setping }) {
  ...
  return (
    <div style={{ position: "relative", borderRadius: 8 }}>
      {/* Que ladmin supprime les animaux de la carte*/}
      {user?.category === "admin" && (
        <button
          style={{
            border: "none",
        ...
        <div className="animal-card">
          <div>
            {user?.category !== "admin" &&
              ...
              <svg
              // like pour card que lorque tu connecte 
                onClick={(e) => {
                  if (!user) {
                    e.stopPropagation();
                    e.preventDefault();
                    Swal.fire({
                      ...
                      text: "Vous devez être connecté pour aimer un animal.",
          ...
          <div className="animal-sec">
            <div className="animal-desc">
              <div
                style={{
                  background: animal?.adoption ? "#ef4444" : "#1dc693", 
                  borderRadius: 20,
                  width: animal?.adoption ? "110px" : "80px", 
                  height: 28,
                  color: "white",
                  position: "relative",
                  top: -12,
              ...
              >
                <span>{animal?.adoption ? "Non disponible" : "Disponible"}</span>
              </div>
            </div>
            <h2>{animal.name}</h2>
            <span className="h1name">{animal.age}</span>
            <div style={{ width: 202, height: 26, background: "#fbf7ff", marginBottom: 5, borderRadius: 8, display: "flex", justifyContent: "center", alignItems: "center" }}>
              ...
              <span style={{ color: "#533778" }} >{animal.race}</span></div>
